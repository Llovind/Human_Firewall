...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const res = await fetch(`${apiUrl}/api/admin/gophish/pages/${id}`, {
      method: 'PUT',
      headers: {
    ...
    if (!res.ok) {
      return NextResponse.json({ error: data.error || 'Gagal memperbarui landing page' }, { status: res.status });
    }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      return NextResponse.json({ error: data.error || 'Gagal menghapus landing page' }, { status: res.status });
    }
