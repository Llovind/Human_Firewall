...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(`${apiUrl}/api/user-activity?email=${encodeURIComponent(email)}&token=${encodeURIComponent(token)}`, {
      method: 'GET',
    });

    const data = await res.json();
    if (!res.ok) {
      return NextResponse.json({ error: data.error || 'Gagal mengambil aktivitas' }, { status: res.status });
    }
